...
kpi_data = [
    ...
    },
    {
        ...
        "custom_render": True, # Flag for custom rendering
...
def render_waste_alert_kpi(col, data, delay):
    ...
    with col:
        ...
        # 2. Add an empty metric component ONLY for the tooltip (help text) if required by Streamlit's layout structure,
        # but hide the label/value/delta entirely via visibility and use a fixed key.
        # Note: To avoid the empty space, we just render the HTML and skip the st.metric() call entirely in the loop.
...
for i, data in enumerate(kpi_data):
    ...
    if data.get("custom_render"):
        # Custom Render the Waste KPI (This function only renders HTML and *avoids* the st.metric() call)
        ...
        with kpi_cols[i]:
            ...
            # Standard st.metric call for Tooltip/Layout purposes (but using the label from the HTML block)
